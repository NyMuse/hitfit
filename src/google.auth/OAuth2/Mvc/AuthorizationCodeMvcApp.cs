...
namespace hitfit.google.auth.Mvc
{
    /// <summary>
    ...
    public class AuthorizationCodeMvcApp : AuthorizationCodeWebApp
    {
        ...
        public AuthorizationCodeMvcApp(Controller controller, FlowMetadata flowData)
            : base(
            flowData.Flow,
            GetUri(controller.Request, flowData.AuthCallback),
            controller.Request.QueryString.Value)
        {
            this.controller = controller;
            this.flowData = flowData;
        }

        //// controller.Request.QueryString.ToUriComponent() .Url.GetLeftPart(UriPartial.Authority)
        private static string GetUri(HttpRequest request, string authCallback)
        {
            var builder = new UriBuilder();
            builder.Scheme = request.Scheme;

            if (request.Host.Value.Split(':').Length > 1)
            {
                builder.Host = request.Host.Value.Split(':')[0];
                builder.Port = int.Parse(request.Host.Value.Split(':')[1]);
            }
            else
            {
            ...
            builder.Path = authCallback;
            //builder.Query = authCallback;
